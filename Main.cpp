...
void loop(SDL_Renderer* renderer, TTF_Font* sans) {
    SDL_Event event;
    ...
    while (is_running) {
        ...
        game->update();
        //rendering bat, ball, text and particles
        game->render(renderer);
        game->renderParticles(renderer);
        game->renderText(renderer, sans);
...
int run_game() {
    ...
    //opens the font and sets its size for use later to render text
    TTF_Font* sans = TTF_OpenFont("resources/Sans.ttf", 72);

    if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, MIX_DEFAULT_CHANNELS, 4096) < 0)
    {
        std::cout << "Failed to init SDL Mixer" << Mix_GetError << std::endl;
    }
